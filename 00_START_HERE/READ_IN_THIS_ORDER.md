# Read In This Order

Use this order whenever you or any model opens Jawab again.

## 1. Orientation

- `00_START_HERE/jawab_README.md`
- `00_START_HERE/CURRENT_START_HERE.md`

## 2. Master Plan

- `01_Strategy/Jawab_SUPERCHARGED_SOURCE_OF_TRUTH.md`

This is the current source of truth for pricing, products, GTM, scripts, onboarding, churn, expansion, and tool order.

## 3. Daily Execution

- `01_Strategy/Jawab_EXECUTION_DASHBOARD.html`
- `01_Strategy/Jawab_CONTROL_TOWER.md`
- `01_Strategy/Jawab_EXECUTION_PLAYBOOK.md`
- `01_Strategy/kpi_scoreboard.md`

Open the dashboard first when you are actively working. Open the source-of-truth first when you are making decisions.

## 4. Outreach

- `02_Outreach_and_Data/01_READY_TO_SEND_TOP30_WORKING.csv`
- `02_Outreach_and_Data/Jawab_Morning_Send_Pack.md`
- `02_Outreach_and_Data/outreach_messages.md`
- `02_Outreach_and_Data/discovery_call_script.md`
- `02_Outreach_and_Data/objection_handling.md`
- `02_Outreach_and_Data/partnership_outreach.md`

## 5. Assets And Templates

- `02_Outreach_and_Data/clinic_leak_audit_template.csv`
- `02_Outreach_and_Data/lost_patient_calculator.csv`
- `02_Outreach_and_Data/4_week_content_calendar.md`
- `03_Demo_and_Assets/jawab_demo.html`
- `03_Demo_and_Assets/demo_agent_script.md`

## 6. Delivery And Client Ops

- `04_System_and_Skills/Jawab_MAKE_BLUEPRINT.md`
- `06_Client_Ops/jawab_service_agreement.md`
- `06_Client_Ops/jawab_data_processing_addendum.md`
- `06_Client_Ops/jawab_founding_client_letter.md`
- `06_Client_Ops/jawab_invoice_template.md`

## 7. Agent And Skill System

- `04_System_and_Skills/Jawab_AGENT_OPERATING_MODEL.md`
- `04_System_and_Skills/SKILL_INDEX.md`
- `04_System_and_Skills/EXTERNAL_SKILL_INTAKE.md`

## Do Not Use As Current Truth

Do not use anything in:

- `99_Outdated_V2`

That folder contains old pricing, old voice-heavy positioning, exploratory plans, and historical context only.
