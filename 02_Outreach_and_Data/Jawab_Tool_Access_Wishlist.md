# Jawab Tool Access Wishlist

This is the minimum tool and account stack worth connecting.

## Must-Have Now

### WhatsApp Business

Use your Egyptian number here for manual outreach.

Why now:

- fastest way to send
- no extra backend
- immediate business-facing identity

### Google Workspace

Use one Jawab-dedicated Gmail, Drive, and Sheets base.

Why now:

- outreach tracking
- reply handling
- warm intro management
- simple CRM surface

### Loom

Use for the 60-second demo link.

Why now:

- zero-friction sharing
- better than shipping local files around

## Useful Soon

### Twilio

Use later for the actual recovery workflow and WhatsApp sender setup.

Why soon:

- official WhatsApp infrastructure
- future delivery layer

### Make

Use later for the first no-code workflow.

Why soon:

- faster than self-hosting n8n
- enough for the first recovery scenarios

### Apify

Use for public-web enrichment after the first manual batch is working.

Why soon:

- Google Maps scraping
- Instagram/business profile enrichment
- scaling research without full manual repetition

### Vercel

Use if you want the HTML demo hosted on a clean live URL.

Why soon:

- easier demo sharing
- better trust than a local file

## Useful Later

### Airtable or HubSpot Free

Use only when Sheets becomes too messy.

### Clay

Use only if enrichment becomes a real bottleneck and the model is already getting replies.

## Do Not Spend Time On Now

- local n8n setup
- custom backend rewrites
- Apollo
- Instantly
- advanced CRM migrations
- voice telephony experiments

## Best Connection Order

1. `WhatsApp Business`
2. `Google Workspace / Gmail / Sheets`
3. `Loom`
4. `Twilio`
5. `Make`
6. `Apify`
7. `Vercel`
8. `Airtable or HubSpot`
9. `Clay`

## If You Want To Plug Things Into Me

Highest-value connections or inputs:

- Gmail access for outreach and warm-intro organization
- Apify outputs for large-batch lead enrichment
- Twilio account details once we start real recovery workflow setup
- Loom links or exported demo assets

Lowest-value right now:

- local automation tools
- more code-only infrastructure
- extra plugins that do not directly improve outreach or proof
